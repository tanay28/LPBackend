...
module.exports = {
    ...
    login : async (req, res, next) => {
        ...
        if (username.includes('@')) {
            ...
            try {
                const userCredential = await Users.findOne({ where: { email : username } }).catch((err) => {
                ...
                if (userCredential && userCredential != null) {
                    ...
                    if (result) {
                        ...
                        logger.logActivity(loggerStatus.ERROR, username, 'login successfully with email', null, OPERATIONS.AUTH.LOGIN);
                        res.json({
                            status: 200,
                ...
                logger.logActivity(loggerStatus.ERROR, username, 'Unable to execute db query to select', error, OPERATIONS.AUTH.LOGIN);
                return;
            }
        } else {
            // Find credential with user phone no as username
            try {
                const userCredential = await Users.findOne({ where: { phoneNo : username } }).catch((err) => {
                    logger.logActivity(loggerStatus.ERROR, req.body, 'Unable to fetch data from DB', err, OPERATIONS.AUTH.LOGIN);
                });
                ...
                if (userCredential && userCredential != null) { 
                    ...
                    if (result) {
                        ...
                        logger.logActivity(loggerStatus.ERROR, username, 'login successfully with Phone no', null, OPERATIONS.AUTH.LOGIN);
                        res.json({
                        status: 200,
                        message: "login successfully",
                        token: jsontoken
